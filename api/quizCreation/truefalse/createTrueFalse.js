...
const createTrueFalse = async (quizID, questions) => {
    try {
        ...
        // Send the request
        const response = await fetch(`${config.API_URL}quizzes/true-false`, {
            method: 'POST',
            headers: {
        ...
        if (!response.ok) {
            throw new Error('Failed to create True/False question');
        }
